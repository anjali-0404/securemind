...
try:
    ...
    y = df['risk_level'].astype(str).values  # Convert to numpy array immediately
    
    # Synthetic predictions (with slight error)
    y_pred = y.copy()  # Copy the array
    # Introduce 5-6% error
...
explode = tuple([0.05] * len(class_counts))  # Dynamic explode for any number of classes

plt.pie(class_counts, labels=[f'{label.capitalize()}\n({count:,} samples)' 
                               for label, count in class_counts.items()],
        autopct='%1.1f%%', colors=colors_pie[:len(class_counts)], explode=explode,
